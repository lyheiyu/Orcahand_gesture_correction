...
\caption{Paired accuracy differences across the same 20 three-shot supervised sequence splits. Values above zero favor Optimized Action. Improvements over Raw are consistent across classifiers, whereas the smaller gains over Corrected reveal the remaining uncertainty associated with the current dataset size.}
...
\begin{table}[!t]
\centering
\caption{Loss ablation in actuator space. Stability values are averaged over 56 sequences; classification values are mean $\pm$ standard deviation over the same 20 three-shot supervised splits. Lower stability values indicate smoother trajectories.}
