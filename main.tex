...
The subset contains 56 gesture sequences and 7109 frames. For each repeat, we first create a stratified sequence-level split with 20\% of the sequences held out for testing. We then retain three sequences per class from the remaining training partition, resulting in 18 training sequences and 12 test sequences in each repeat; unused candidates in the training partition do not enter model fitting. This process is repeated 20 times using seeds 42--61, and the same sequence split is used for every representation and classifier within a repeat. In this paper, \emph{few-shot} therefore refers operationally to a three-training-sequence-per-class supervised evaluation protocol; it does not denote episodic meta-learning. Because the current dataset is moderately imbalanced, we report macro-F1 and Cohen's kappa in addition to accuracy. The class distribution is summarized in Table~\ref{tab:dataset_summary}.
...
\caption{Class distribution of the controlled six-class Chinese dance gesture subset. The prototype dataset is moderately imbalanced, which motivates reporting macro-F1 and Cohen's kappa in addition to accuracy.}
...
The classifier comparison suggests two complementary observations. First, representation choice matters strongly: all four classifiers improve substantially when moving from raw landmarks to structured actuator-space features. Second, the refined actuator representation gives consistent gains over Raw across the evaluated lightweight learners rather than being tied to one specific classifier. In the current six-class setting, SVM provides the strongest overall result for Optimized Action, while KNN, RandomForest, and MLP show the same broad ranking trend among the non-PCA structured representations.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{paper_rewrite_main/performance_heatmaps_6class.png}
\caption{Mean accuracy, macro-F1, and Cohen's kappa across classifiers and the non-PCA representations shown in this panel. Optimized Action has the highest mean accuracy in every classifier column within this comparison, whereas the separate PCA-17 comparison is reported in Table~\ref{tab:pca_comparison} and Figure~\ref{fig:pca_comparison}. Optimized Full generally loses discriminative information after reconstruction into the higher-dimensional landmark space.}
...
\textbf{RQ1: Temporal stability.} Within actuator space, Optimized Action reduces mean velocity from 0.5601 to 0.3015 and mean acceleration from 0.8687 to 0.3206 relative to Corrected. The temporal ablation further shows that removing acceleration regularization or both temporal terms increases second-order variation. The current evidence therefore indicates that the refinement performs its intended stabilization role.
...
\textbf{RQ3: Simpler explanations.} The result cannot be reduced to a universal claim of superior smoothing or dimensionality reduction. Kalman filtering produces the lowest landmark-space velocity and acceleration, while PCA-17 is stronger under RandomForest and MLP. Nevertheless, Optimized Action provides higher RandomForest accuracy and Cohen's kappa than the smoothing baselines and gives the strongest SVM and KNN results in the PCA comparison. Thus, the proposed representation is competitive and preserves useful structured information, but it does not dominate every alternative under every classifier.

\textbf{RQ4: Component roles and cost.} Temporal regularization primarily improves stability rather than classification accuracy in the present ablation. Palm-normal alignment provides a larger observable contribution to SVM recognition, while replacing Huber fitting with $L_2$ increases solve time without improving accuracy. The full solver requires 29.9 ms per frame on average and 37.3 ms at the 95th percentile in the reported CPU benchmark. These results clarify the role of each component without implying that the current fixed weights are globally optimal.
